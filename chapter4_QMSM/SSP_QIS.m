...
lambda = 11.32; %for 11.17 11.2 11.23 11.26 11.29 11.32  
mu = 5.9101;  %for 7.25 7 6.75 6.5 6.25 6  %mu=5.9101 ~ 6, F(mu)=502.3490 from model solution and mu=7.25 from observed data
alpha = 0.029; %for &0.024 &0.025 &0.026 &0.027 &0.028 &0.029%%Observed data 2.58/89=0.029  
r = 0.4;  %for &0.9 &0.8 &0.7 &0.6 &0.5 &0.4    
xi = 0.016; %&0.019 &0.018 &0.017 &0.016 &0.015 &0.016 %%Observed data 1.33/89=0.015
eta = 0.0588;  %%from observed data
%phi =0.03;
% Calculate intermediate variables (using symsum for L)
rho_star = lambda * (1 + beta) / (mu + eta);
rho_starstar = lambda * (1 + beta) / mu;
rho = rho_star + rho_starstar;
L = 1 + rho + symsum((rho^n)/factorial(n), n, 2, C-D);

% Calculate b_i values recursively
b = sym('b', [1, N]);
b(1:C-D) = 1;
b(C-D+1:N) = (N - (C-D:N-1)) / N;
for i = C-D+1:N-1
    b_i_star = b(i-1) / (mu + eta + (i - (C-D)) * alpha * (1 - r) + xi);
    b_i_starstar = b(i-1) / (mu + (i - (C-D)) * alpha * (1 - r) + xi);
    b(i) = b_i_star + b_i_starstar;
end

% Calculate all probabilities using symsum
p_0_0_0 = ((L + symsum((lambda * (1 + beta))^n / (mu + eta + xi) * prod(b(C-D+1:N-1)), n, C-D, N-1) + (lambda * (1 + beta))^N / (mu + eta + xi) * prod(b(C-D+1:N-1))))^-1;
p_0_1_s = rho * p_0_0_0;
p_0_n_s = (rho^n * p_0_0_0) / factorial(n);  % For n = 2 to C-D
p_1_1_s = (lambda * (1 + beta)) / (mu + eta + xi) * prod(b(2:N-1)) * p_0_0_0;
p_1_n_s = (lambda * (1 + beta))^n / (mu + eta + xi) * prod(b(C-D+1:N-1)) * p_0_0_0;
p_1_N_s = (lambda * (1 + beta))^N / (mu + eta + xi) * prod(b(C-D+1:N-1)) * p_0_0_0;
...
Ls = double(0*(p_0_0_0)+1*(p_0_1_s))+sum(sum(n .* p_0_n_s_value, 2), C-D-1) + sum(sum(n .* p_1_n_s_value, C-D), N-1);
Lq = double(p_0_1_s)+sum(sum((n - (C - D)) .* p_1_n_s_value, C-D+2), N-1)+(N-C+D)*double(p_1_N_s);
Ws = Ls / lambda_eff;
Wq = Lq / lambda_eff;
PB = sum(sum(p_0_n_s_value, C-D), C-D)+sum(sum(p_1_n_s_value(2:end), 2), 2);
PI = double((p_0_0_0)+(p_0_1_s))+sum(sum(p_0_n_s_value(1:end-1), 2), 2) + double(p_1_N_s);
...
F = C1 * EI + C2 * Er + C3 * E0 * Er + C4 * VLr + C5 * PB + C6 * PI + C7 * Lq + C8 * GR + C9 * LR;
...
% %disp([num2str(LS)])
% %disp([num2str(LSc)])
% %disp([num2str(s)])  % Replace with the calculated value of s
% %disp([num2str(alpha_1)])
% disp([num2str(beta1)])
% disp([num2str(lambda_eff)])
% disp([num2str(Ls)])
% disp([num2str(Lq)])
% disp([num2str(Ws)])
% disp([num2str(Wq)])
