function Cost_Function=F_Catastrophes(~)  
...
beta = 0.3;
lambda = 11.17;
%mu = 7.25; from observed data, Optimal(mu)=5.901 in Chapter 5 
alpha = 0.029;
r1 = 0.4;
xi = 0.015;
eta = 0.0588;
...
%%%n=C-D:1:N; (4-10) N=10.
P_000 =(((1+(lambda*(1+beta)/new_mu+kappa+lambda/new_mu+kappa+eta)+symsum(((factorial(n))^-1*(lambda*(1+beta)/new_mu+kappa+lambda*(1+beta)/new_mu+kappa+eta)^n),n,2,C-D)+symsum((lambda*(1+beta))^n1/(new_mu+kappa+eta+xi)*symprod((N-r+1)/(N*((new_mu+kappa+eta+xi)+(r-(C-D+1))*alpha*(1-r1))),r,C-D+1,N),n1,C-D+1,N))))^-1;
P_0ns =(((factorial(n))^-1)*(lambda*(1+beta)/new_mu+kappa+lambda*(1+beta)/(new_mu+kappa+eta))^n)*P_000; %% n=1:1:C-D
P_0C_Ds=(((factorial(C-D))^-1)*(lambda*(1+beta)/new_mu+kappa+lambda*(1+beta)/(new_mu+kappa+eta))^(C-D))*P_000;
P_1ns =((((lambda*(1+beta))^n1)/(new_mu+kappa+eta+xi))*(symprod((N-r+1)*(1-gamma)/(N*((new_mu+kappa+eta+xi)+(r-(C-D+1))*alpha*(1-r1))),r,C-D+1,N)))*P_000; %% n=C-D:1:N
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
LR=(BR+RR);
GR=Ls-LR;
Ec =symsum(s*(1-gamma)*P_0ns,n,0,C-D-1)+symsum(s*(1-gamma)*P_1ns,n,C-D,N);
%%%%%%Expected Total Cost Function Interms of parameter "mu"%%%%%%%%%%%%%.
Cost_Function=C1 * EI + C2 * Er + C3 * E0 * Er + C4 * VLr + C5 * P_B + C6 * P_I + C7 * Lq + C8 * GR + C9 * LR+C10*Ec;
end
